...
/// Transcribe in-memory mono f32 PCM already at 16 kHz (whisper.cpp native rate).
///
/// Skips disk I/O — intended for live chunk pipelines that already hold resampled
/// samples in memory.
pub fn transcribe_pcm_16k_mono(
    samples: &[f32],
    config: &Config,
) -> Result<TranscribeResult, TranscribeError> {
    transcribe_pcm_16k_mono_with_hints(samples, config, &DecodeHints::default())
}

/// Like [`transcribe_pcm_16k_mono`] with optional decode hints.
pub fn transcribe_pcm_16k_mono_with_hints(
    samples: &[f32],
    config: &Config,
    hints: &DecodeHints,
) -> Result<TranscribeResult, TranscribeError> {
    ...
    match config.transcription.engine.as_str() {
        "whisper" => transcribe_whisper_pcm_dispatch(samples, config, hints),
        other => {
            tracing::warn!(
                engine = other,
                "in-memory transcription only supports whisper — falling back"
            );
            transcribe_whisper_pcm_dispatch(samples, config, hints)
        }
    }
}

#[cfg(feature = "whisper")]
fn transcribe_whisper_pcm_dispatch(
    samples: &[f32],
    config: &Config,
    hints: &DecodeHints,
) -> Result<TranscribeResult, TranscribeError> {
    static MEMORY_PATH: std::sync::LazyLock<std::path::PathBuf> =
        std::sync::LazyLock::new(|| std::path::PathBuf::from("<memory>"));
    with_cached_whisper_context(config, |ctx| {
        transcribe_whisper_samples(ctx, samples, &MEMORY_PATH, config, hints)
    })
}
...
fn transcribe_whisper_chunk_ranges(
    ...
    with_cached_whisper_context(config, |ctx| {
        transcribe_whisper_chunk_ranges_with_context(
            ctx,
            samples,
            chunk_ranges,
            audio_duration_secs,
...
#[cfg(feature = "whisper")]
fn transcribe_whisper_chunk_ranges_with_context(
...
/// Set once a GPU-backed whisper context fails to initialize at runtime. When
/// this is true, all subsequent contexts are created on the CPU. This lets us
/// gracefully fall back (e.g. Metal/CUDA unavailable in the current runtime)
/// instead of failing every transcription with "Failed to create a new whisper
/// context".
#[cfg(feature = "whisper")]
pub(crate) static WHISPER_GPU_DISABLED: std::sync::atomic::AtomicBool =
...
pub(crate) fn whisper_context_params() -> whisper_rs::WhisperContextParameters<'static> {
    ...
    let gpu_compiled = cfg!(any(
        ...
        feature = "metal",
        feature = "vulkan",
    )) && !WHISPER_GPU_DISABLED.load(std::sync::atomic::Ordering::Relaxed);
...
#[cfg(feature = "whisper")]
fn create_whisper_context(model_str: &str) -> Result<whisper_rs::WhisperContext, TranscribeError> {
    ...
    let gpu_compiled = cfg!(any(
        ...
        feature = "metal",
        feature = "vulkan",
    )) && !WHISPER_GPU_DISABLED.load(Ordering::Relaxed);

    // On Apple Silicon with Metal compiled in, prefer GPU for live-chunk throughput.
    // Other platforms (and macOS after a GPU init failure) use CPU first for reliability.
    #[cfg(all(target_os = "macos", target_arch = "aarch64", feature = "metal"))]
    if gpu_compiled {
        match whisper_rs::WhisperContext::new_with_params(model_str, whisper_context_params()) {
            Ok(ctx) => return Ok(ctx),
            Err(err) => {
                tracing::warn!(error = %err, "Metal whisper context creation failed; falling back to CPU");
                WHISPER_GPU_DISABLED.store(true, Ordering::Relaxed);
            }
        }
    }
    ...
    if let Ok(ctx) = whisper_rs::WhisperContext::new_with_params(model_str, cpu_params) {
        return Ok(ctx);
    }

    // Non-Apple-Silicon: try GPU when CPU init fails (covers CPU-only misbuilds).
    #[cfg(not(all(target_os = "macos", target_arch = "aarch64", feature = "metal")))]
    if gpu_compiled {
        match whisper_rs::WhisperContext::new_with_params(model_str, whisper_context_params()) {
            Ok(ctx) => return Ok(ctx),
            Err(err) => {
                tracing::warn!(error = %err, "GPU whisper context creation failed");
                WHISPER_GPU_DISABLED.store(true, Ordering::Relaxed);
            }
        }
    }
    ...
    let mut cpu_params = whisper_rs::WhisperContextParameters::default();
    cpu_params.use_gpu = false;
    whisper_rs::WhisperContext::new_with_params(model_str, cpu_params)
        .map_err(|e| TranscribeError::ModelLoadError(format!("{}", e)))
}

/// Verify that whisper can parse the configured model weights. Used after
/// downloads to catch same-size corruption that byte-count checks miss.
#[cfg(feature = "whisper")]
pub fn probe_whisper_model(config: &Config) -> Result<(), TranscribeError> {
    preload_whisper_model(config)
}

#[cfg(not(feature = "whisper"))]
pub fn probe_whisper_model(_config: &Config) -> Result<(), TranscribeError> {
    Ok(())
}
...
fn load_whisper_context(config: &Config) -> Result<whisper_rs::WhisperContext, TranscribeError> {
    ...
    create_whisper_context(model_str)
}

// ── Whisper context cache ──────────────────────────────────────
//
// Loading ggml-small.bin takes several seconds on first use. Reuse one context
// across DeskSec chunk transcriptions and preload it at app startup so the first
// Record press does not block on model I/O.
...
static WHISPER_CONTEXT_CACHE: std::sync::LazyLock<
    std::sync::Mutex<Option<CachedWhisperContext>>,
> = std::sync::LazyLock::new(|| std::sync::Mutex::new(None));

#[cfg(feature = "whisper")]
fn whisper_cache_key(config: &Config) -> Result<String, TranscribeError> {
    let model_path = resolve_model_path(config)?;
    Ok(format!(
        "{}:{}",
...
#[cfg(feature = "whisper")]
fn with_cached_whisper_context<R>(
    config: &Config,
    f: impl FnOnce(&whisper_rs::WhisperContext) -> Result<R, TranscribeError>,
) -> Result<R, TranscribeError> {
    let key = whisper_cache_key(config)?;
    let mut cache = WHISPER_CONTEXT_CACHE
        .lock()
        .map_err(|_| TranscribeError::ModelLoadError("whisper cache lock poisoned".into()))?;
    if cache
        .as_ref()
        .map(|cached| cached.cache_key != key)
        .unwrap_or(true)
    {
        tracing::info!(model = %config.transcription.model, "loading whisper model into cache");
    ...
    f(&cache.as_ref().expect("cache populated above").ctx)
}

/// Warm the whisper model cache so the first transcription chunk does not pay
/// the full load cost. Safe to call repeatedly and from a background thread.
#[cfg(feature = "whisper")]
pub fn preload_whisper_model(config: &Config) -> Result<(), TranscribeError> {
    with_cached_whisper_context(config, |_| Ok(()))
}

#[cfg(not(feature = "whisper"))]
pub fn preload_whisper_model(_config: &Config) -> Result<(), TranscribeError> {
    Ok(())
}

/// Drop a cached whisper context (e.g. after the user changes models).
#[cfg(feature = "whisper")]
pub fn invalidate_whisper_context_cache() {
...
mod tests {
    ...
    #[test]
    #[cfg(feature = "whisper")]
    fn load_real_whisper_models_if_present() {
        let home = dirs::home_dir().expect("home dir");
        for model in ["tiny", "base", "small"] {
            let path = home.join(format!(".minutes/models/ggml-{model}.bin"));
            if !path.exists() {
                continue;
            }
            let mut config = Config::default();
            config.transcription.model = model.into();
            let result = load_whisper_context(&config);
            assert!(
                result.is_ok(),
                "{model}: {}",
